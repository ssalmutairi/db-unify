...
const options = {
	host: "localhost",
	port: 54322, // Update this as per your setup
	dialect: "postgres", // Change this to test other dialects
	username: "test",
	password: "test",
	logging: false,
};
...
test("listTables", async (t) => {
	const tables = await dbUnify.listTables("information_schema"); // Update database name
...
test("createUser", async (t) => {
	// await dbUnify.createUser("newUser", "newPassword"); // You might want to handle cleanup after test
...
test("grantPrivileges specific table", async (t) => {
	await dbUnify.grantPrivileges("test", "test", "SELECT,INSERT", "demo"); // Update accordingly
	// Here, you can further query the database to check if the privileges were really granted
	t.pass("Privileges granted successfully");
});

test("revokePrivileges specific table", async (t) => {
	await dbUnify.revokePrivileges("test", "test", "SELECT,INSERT", "demo"); // Update accordingly
	// Here, you can further query the database to check if the privileges were really revoked
	t.pass("Privileges revoked successfully");
});
